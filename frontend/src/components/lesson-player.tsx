...
export function LessonPlayer({ lessonId }: { lessonId: number }) {
  ...
  const [currentUserRole, setCurrentUserRole] = useState<'student' | 'teacher' | 'admin' | 'superadmin' | null>(null)
  ...
  useEffect(() => {
    api<{ lesson: LessonDetail; progress: ProgressItem; viewer_role: 'student' | 'teacher' | 'admin' | 'superadmin' }>(`/lessons/${lessonId}`, undefined, true)
      .then((data) => {
        setCurrentUserRole(data.viewer_role)
        setLesson({
          ...data.lesson,
          module: {
            ...data.lesson.module,
