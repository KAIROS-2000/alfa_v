      ...
      <div className="relative mx-auto mt-8 max-w-5xl pb-8 pt-4">
        ...
        <div className="space-y-10">
          {entries.map(({ key, module, lesson, side }) => {
            ...
            return (
              <div key={key} className="roadmap-node relative flex items-start justify-center sm:items-center">
                <div className={`absolute left-1/2 top-1/2 hidden h-1 w-[17%] -translate-y-1/2 rounded-full bg-slate-200 sm:block ${side === 'left' ? '-translate-x-full' : ''}`} />
                <div className={`flex w-full items-center justify-start pl-12 sm:pl-0 ${side === 'left' ? 'sm:justify-start sm:pr-[52%]' : 'sm:justify-end sm:pl-[52%]'}`}>
                  <div className={`w-full min-w-0 max-w-full rounded-[26px] border border-white/70 p-4 shadow-xl sm:max-w-sm sm:p-5 ${isLocked ? 'bg-slate-100/90' : 'bg-white/90'}`}>
                    <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
                      <p className="min-w-0 flex-1 break-words text-[11px] font-bold uppercase leading-4 tracking-[0.18em] text-slate-500 sm:text-xs">{module.title}</p>
                      <span className="shrink-0 rounded-full px-3 py-1 text-center text-xs font-bold text-white" style={{ backgroundColor: module.color }}>{module.age_group}</span>
                    </div>
                    <h3 className="mt-3 break-words text-lg font-black leading-tight text-slate-900 sm:text-xl">{lesson.title}</h3>
                    <p className="mt-2 break-words text-sm leading-6 text-slate-600">{lesson.summary}</p>
                ...
                <div className={`absolute left-5 top-6 flex h-14 w-14 -translate-x-1/2 items-center justify-center rounded-full ring-4 shadow-lg sm:left-1/2 sm:top-1/2 sm:h-24 sm:w-24 sm:-translate-y-1/2 sm:ring-8 ${bubbleClass}`}>
                  <div className="w-full px-1 text-center leading-none sm:px-2">
                    <div className="text-xl font-black sm:text-2xl">{lesson.order_index}</div>
