...
def has_explicit_code_task_intent(
    *,
    ...
    has_action = any(
        _contains_token(normalized, token) for token in CODE_INTENT_ACTION_TOKENS
    )
    has_marker = any(
        snippet in normalized for snippet in CODE_INTENT_MARKER_SNIPPETS
    ) or any(_contains_token(normalized, token) for token in CODE_INTENT_MARKER_TOKENS)
...
def normalize_task_type(value: str | None, *, age_group: str | None = None) -> str:
    ...
    return (
        "code"
        if normalized == "code" and age_group_supports_code(age_group)
        else "text"
    )
...
def normalize_assignment_type(value: str | None) -> str:
    ...
    return (
        normalized if normalized in VALID_ASSIGNMENT_TYPES else DEFAULT_ASSIGNMENT_TYPE
    )
...
def normalize_submission_format(value: str | None) -> str:
    ...
    return (
        normalized
        if normalized in VALID_SUBMISSION_FORMATS
        else DEFAULT_SUBMISSION_FORMAT
    )
...
def normalize_code_language(value: str | None, default: str | None = None) -> str:
    ...
    return (
        normalized
        if normalized in VALID_CODE_LANGUAGES
        else (default or DEFAULT_CODE_LANGUAGE)
    )
...
def normalize_task_evaluation_mode(
    ...
    *,
    ...
    if has_keywords and not is_custom_lesson:
        return "keywords"
    return (
        DEFAULT_TASK_EVALUATION_MODE
        if is_custom_lesson
        else ("keywords" if has_keywords else DEFAULT_TASK_EVALUATION_MODE)
    )
...
def normalize_task_validation(
    ...
    *,
    ...
    default_language = (
        "javascript"
        if (age_group or "").strip().lower() == "senior"
    ...
    return {
        ...
        "language": (
            language
            if effective_task_type == "code" or evaluation_mode == "stdin_stdout"
            else None
        ),
        "keywords": keywords,
        "tests": tests,
        "time_limit_ms": _safe_int(
            raw.get("time_limit_ms"), 2000, minimum=500, maximum=10000
        ),
        "memory_limit_mb": _safe_int(
            raw.get("memory_limit_mb"), 128, minimum=32, maximum=1024
        ),
    }
...
def public_task_validation(
    ...
    *,
    ...
    return {
        ...
        "time_limit_ms": (
            normalized["time_limit_ms"]
            if normalized["runner"] == "stdin_stdout"
            else None
        ),
        "memory_limit_mb": (
            normalized["memory_limit_mb"]
            if normalized["runner"] == "stdin_stdout"
            else None
        ),
    }
...
def encode_assignment_description(
    ...
    return (
        f"{ASSIGNMENT_META_PREFIX}{payload}\n{body}"
        if body
        else f"{ASSIGNMENT_META_PREFIX}{payload}"
    )
...
def decode_assignment_description(raw_description: str | None) -> tuple[dict, str]:
    ...
    assignment_type = normalize_assignment_type(
        meta.get("assignment_type") if isinstance(meta, dict) else None
    )
    submission_format = normalize_submission_format(
        meta.get("submission_format") if isinstance(meta, dict) else None
    )
    return (
        {
...
class Task(db.Model):
    ...
    def normalized_validation(self, include_private: bool = False) -> dict:
        payload = normalize_task_validation(
            self.validation,
            is_custom_lesson=bool(
                self.lesson and self.lesson.module.is_custom_classroom_module
            ),
    ...
    def requires_teacher_review(self) -> bool:
        return bool(
            self.lesson
            and self.lesson.module.is_custom_classroom_module
            and self.normalized_validation(include_private=True)["evaluation_mode"]
            == "manual"
        )
...
class ClassMembership(db.Model):
    ...
    __table_args__ = (
        UniqueConstraint(
            "classroom_id", "student_id", name="uq_class_membership_classroom_student"
        ),
...
class AssignmentSubmission(db.Model):
    ...
    __table_args__ = (
        ...
        Index(
            "ix_assignment_submission_assignment_submitted",
            "assignment_id",
            "submitted_at",
        ),
        Index(
            "ix_assignment_submission_student_submitted", "student_id", "submitted_at"
        ),
    )
...
class UserAchievement(db.Model):
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "achievement_id", name="uq_user_achievement_user_achievement"
        ),
